...
filepath() {
  ...
  echo ${FILETYPE_LOWER}/${FILETYPE}_${YEAR}${EXTENTION}
}
...
pre_process() {
  FILETYPE_LOWER=$(datatype_to_path $FILETYPE)
  mkdir -p ${DOWNLOAD_FILE_OUTPUT_DIRECTORY}${FILETYPE_LOWER}/${YEAR}
}

download() {
  FILETYPE_LOWER=$(datatype_to_path $FILETYPE)
  curl -u ${JRDB_USER}:${JRDB_PASSWORD} ${JRDB_BASE_URL}$(filepath) -L -o ${DOWNLOAD_FILE_OUTPUT_DIRECTORY}${FILETYPE_LOWER}/${YEAR}/tmp${EXTENTION}
}

decompression() {
  FILETYPE_LOWER=$(datatype_to_path $FILETYPE)
  lha -xw=${DOWNLOAD_FILE_OUTPUT_DIRECTORY}${FILETYPE_LOWER}/${YEAR} ${DOWNLOAD_FILE_OUTPUT_DIRECTORY}${FILETYPE_LOWER}/${YEAR}/tmp${EXTENTION}
}

convert_encoding() {
  # 解凍されたテキストファイルを CP932 から UTF-8 に変換
  FILETYPE_LOWER=$(datatype_to_path $FILETYPE)
  for file in ${DOWNLOAD_FILE_OUTPUT_DIRECTORY}${FILETYPE_LOWER}/${YEAR}/*.txt; do
    if [ -f "$file" ]; then
...
txt_to_csv() {
  # テキストファイルを CSV に変換
  FILETYPE_LOWER=$(datatype_to_path $FILETYPE)
  for txt_file in ${DOWNLOAD_FILE_OUTPUT_DIRECTORY}${FILETYPE_LOWER}/${YEAR}/*.txt; do
    if [ -f "$txt_file" ]; then
      csv_file="${txt_file%.txt}.csv"
      # 固定長テキストをカンマで区切ったCSVに変換
...
post_process() {
  FILETYPE_LOWER=$(datatype_to_path $FILETYPE)
  rm -f ${DOWNLOAD_FILE_OUTPUT_DIRECTORY}${FILETYPE_LOWER}/${YEAR}/*${EXTENTION}
}
