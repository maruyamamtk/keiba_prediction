...
### 1.13 ブリンカー装着状況（`raw.horse_results.blinker` より・Issue #445）

KYF（前日発表）のブリンカー区分（1=初装着, 2=再装着, 3=継続, 空白=非装着）から作る。過去走の集計は当該レースより前の日付のみ（同日除外）で、出走取消の行は集計・前走参照から外す。当日の馬具変更は KYF に反映されないため扱えない（既知の制約）。

| カラム名 | 説明 |
|---|---|
| `blinker_code` | 0=なし/1=初/2=再/3=継続（数値。カテゴリ化は予測時のカテゴリ不一致を避けるため行わない） |
| `blinker_first_flag` / `blinker_on_flag` | 初装着 / 装着あり |
| `blinker_off_flag` | 前走装着 → 今回非装着（外し） |
| `blinker_change_flag` | 前走と装着状態が変化（初・再装着・外し） |
| `blinker_race_count` | 過去の装着出走回数 |
| `blinker_perf_diff` | 過去の装着時平均着順率 − 非装着時（両方1走以上のみ。負=装着時が好成績） |
| `blinker_first_x_prev_loss` | 初装着 × 前走着順率（初装着以外は0） |
| `trainer_first_blinker_te` | 厩舎の初ブリンカー時の3着以内率（過去5年・m=10 平滑化・5走未満は NULL） |
