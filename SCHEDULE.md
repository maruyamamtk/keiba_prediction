...
**概要**: JRDBからその日のデータをダウンロードし、GCS→BigQueryへロードします。当日から直近7日間のファイルを対象とします（実行ごとに一時ディレクトリへ取り直すため、速報版SECも確定版に置き換わる）。

**処理フロー**:
1. JRDBから当日データをダウンロード（lzh解凍・CP932→UTF-8変換）
2. GCSへアップロード（MD5重複チェック）
3. BigQueryへMERGE/UPSERTロード（`raw.load_history` で重複スキップ）
4. 成績欠損チェック（Issue #440）: 8〜35日前の開催日で `race_results` が不完全（IDM NULL率20%超、または出走表の90%未満の行数）な日を検知し、SECをJRDBから再取得→再ロード。再取得に失敗した日があれば `partial`。確定版（開催日+7日以降の取得）をロード済みでも不完全な日（開催途中の中止など）は再取得しない。修復結果は月次再学習の `training_data` 全期間再生成で反映される
