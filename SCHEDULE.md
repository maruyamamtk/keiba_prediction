...
4. 成績欠損チェック（Issue #440）: 7〜35日前の開催日で `race_results` が不完全（IDM NULL率20%超＝速報版のまま、または成績行なし）な日を検知し、SECをJRDBから再取得→再ロード。再取得に失敗した日があれば `partial`（JRDBにSECがない開催中止日は失敗扱いにしない）。修復結果は月次再学習の `training_data` 全期間再生成で反映される
